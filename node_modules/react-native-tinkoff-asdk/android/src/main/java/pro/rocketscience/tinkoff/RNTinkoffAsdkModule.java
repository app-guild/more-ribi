  ...
  public void onActivityResult(final Activity activity, int requestCode, int resultCode, Intent data) {
    ...
    if (bundle != null) {
      ...
      for (String key : bundle.keySet()) {
        log += key + " : " + (bundle.get(key) != null ? bundle.get(key) : "NULL") + "/n";
    ...
    if (requestCode == REQUEST_CODE_PAY) {
      if (resultCode == 500) {
        try {
          for (String key : bundle.keySet()) {
            rejectPromise(((Throwable) bundle.get(key)).getMessage());
          }
        } catch (Throwable t) {
...
//        .setReceipt(createReceipt(options.getArray("Items"), options.getString("Email"), options.getString("Taxation")))
  ...
  @ReactMethod
  public void Charge(ReadableMap options, final Promise promise) {
    Log.d("Notification", "Charge start");
    rejectPromise("Запущен новый процесс оплаты");
    paymentPromise = promise;
    ...
    try {
      ...
      String paymentId = options.hasKey("PaymentId") ? options.getString("PaymentId") : "";
      String rebillId = options.hasKey("RebillId") ? options.getString("RebillId") : "";

      PaymentInfo info = acquiringSdk.charge(Long.parseLong(paymentId), rebillId);
      WritableNativeMap data = new WritableNativeMap();
      data.putString("cardId", info.getCardId());
      data.putString("errorCode", info.getErrorCode());
      data.putString("orderId", info.getOrderId());
      data.putDouble("amount", info.getAmount());
      data.putDouble("paymentId", info.getPaymentId());
